...
@section('content')
    {{-- table --}}
        <x-admin.table  addbutton="{{route('admin.tests.create')}}" deletebutton="{{route('admin.tests.deleteAll')}}">
                ...
                @foreach($rows as $row)
                    <tr class="delete_row">
                        ...
                        <td class="product-action">
                            <span class="action-edit text-primary"><a href="{{route('admin.tests.edit' , ['id' => $row->id])}}"><i class="feather icon-edit"></i></a></span>
                            <span class="delete-row text-danger" data-url="{{url('admin/tests/'.$row->id)}}"><i class="feather icon-trash"></i></span>
                        </td>
                    </tr>
                @endforeach
            </x-slot>
